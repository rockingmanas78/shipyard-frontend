    ...
    return (
      <div className="rounded-xl border bg-white shadow-sm overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          ...
          <div>
            <div className="text-xs font-medium text-gray-600 mb-1">Recommendations <span className="text-gray-400">(use ; between items)</span></div>
      ...
      {obsFilteredSorted.map(({ raw }, idx) => (
        <div key={raw.id} className="rounded-xl border bg-white shadow-sm overflow-hidden">
          <div className="relative">
            ...
            <div className="absolute top-3 left-3 space-y-1">
              <Tag value={raw.condition === "none" && !!raw.tags?.rust_stains ? "rust" : raw.condition === "none" && (raw.recommendations_high_severity_only || []).length ? "attention" : raw.condition} />
            </div>
          </div>
  ...
  <div className="p-5 overflow-x-auto">
    <table className="min-w-full text-sm">
      <thead className="bg-gray-50 text-gray-600 border-b">
        <tr>
          <th className="text-left font-medium px-3 py-2 w-10">#</th>
          <th className="text-left font-medium px-3 py-2 w-32">Photo</th>
          <th className="text-left font-medium px-3 py-2 w-40">Area</th>
          <th className="text-left font-medium px-3 py-2 w-48">Assigned to / Responsible</th>
          <th className="text-left font-medium px-3 py-2 w-36">Category</th>
          <th className="text-left font-medium px-3 py-2 w-[40%]">Description &amp; Recommendations</th>
          <th className="text-left font-medium px-3 py-2 w-36">Targeted Deadline</th>
          <th className="text-left font-medium px-3 py-2 w-24">Actions</th>
        </tr>
      </thead>
      <tbody>
        {hazardRowsDerived.length === 0 ? (
          ...
          hazardRowsDerived.map((row, idx) => (
            <tr key={row.id} className="border-b align-top">
              ...
              {/* Photo thumbnail (unbreakable, keeps aspect) */}
              <td className="px-3 py-2">
                <button
                  type="button"
                  className="block w-28"
                  onClick={() => setSelectedPhoto({ ...row, source: "defect", photoId: row.photoId || row.rawId || row.id })}
                  title="Click to enlarge and view details"
                >
                  <div
                    className="w-28 rounded-lg border bg-gray-100 overflow-hidden"
                    style={{ aspectRatio: "4 / 3", height: "5.5rem" }} // ~h-22; ensures visible even if aspect-ratio unsupported
                  >
                    <img
                      src={
                        row.photoId
      ...
      {(() => {
        ...
        return (
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              <div className="w-full max-h-[74vh] rounded-lg border bg-gray-50 overflow-hidden flex items-center justify-center">
                <img
            ...
            <div className="space-y-3 text-sm">
              {currentId ? (
                <div>
                  ...
                  <div className="mt-0.5 text-gray-900 break-all">{/* hidden filename by request */}</div>
                </div>
              ) : null}
                ...
                <>
                  {selectedPhoto.location ? (
                    <div>
                      ...
                      <div className="mt-0.5 text-gray-900">{selectedPhoto.location}</div>
                    </div>
                  ) : null}
                  {effectiveCond ? (
                    <div>
                ...
                <>
                  ...
                  {selectedPhoto.assignedTo ? (
                    <div>
                      ...
                      <div className="mt-0.5 text-gray-900">{selectedPhoto.assignedTo}</div>
                    </div>
                  ) : null}
                  {selectedPhoto.deadline ? (
                    <div>
                      <div className="text-xs font-medium text-gray-600">Deadline</div>
                      <div className="mt-0.5 text-gray-900">{selectedPhoto.deadline}</div>
                    </div>
                  ) : null}
                </>
              )}
...
{addOpen && (
  <div className="fixed inset-0 bg-black/60 flex items-start justify-center z-[60] overflow-y-auto p-6">
    <div className="bg-white rounded-xl p-6 w-full max-w-2xl relative shadow-xl max-h-[90vh] overflow-y-auto">
      <button
        onClick={() => {
          setAddOpen(false);
        }}
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        <div className="block">
          <span className="text-xs font-medium text-gray-600">Select photo from backend (optional)</span>
          ...
          {addForm.id ? (
            <div className="mt-3">
              ...
              <div className="w-full max-h-[50vh] rounded-lg border bg-gray-50 overflow-hidden flex items-center justify-center">
                <img
          ...
          <div className="mt-3">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 max-h-[240px] overflow-y-auto p-1 border rounded-lg bg-white">
              {perImage.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    {active ? (
                      <span className="absolute inset-0 pointer-events-none ring-2 ring-blue-600 rounded-md" />
                    ) : null}
                  </button>
                );
              })}
